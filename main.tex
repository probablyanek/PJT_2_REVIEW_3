...
\addtobeamertemplate{frametitle}{}{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north east, inner sep=0pt, yshift=0.1cm, xshift=-0.3cm]
      at (current page.north east) {\includegraphics[height=1.1cm]{logo.png}};
  \end{tikzpicture}%
}
...
\title[ML Sensor Fusion]{ML-Based Asynchronous Sensor Fusion for Indoor Localization}
...
\tikzset{
  sysblock/.style={
    ...
    text width=2.2cm, align=center, font=\scriptsize\bfseries
  },
  dataflow/.style={
  ...
  pipeblock/.style={
    ...
    text width=1.8cm, align=center, font=\tiny\bfseries
  },
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.45\textwidth}
			\centering
			\scriptsize
			\begin{tabular}{>{\raggedright}p{1.4cm} >{\centering}p{2cm} >{\centering\arraybackslash}p{2cm}}
				\toprule
...
\begin{frame}
	...
	\begin{enumerate}
		\item Design a \textbf{COTS hardware node} that performs radar + Wi-Fi sensing simultaneously
...
\begin{frame}
	...
	\framesubtitle{All COTS Components}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.55\textwidth}
			\centering
			% TikZ Hardware Connection Diagram
			\begin{tikzpicture}[scale=0.85, transform shape]
				\node[sysblock, fill=blue!12] (radar) {HLK-LD2450\\24\,GHz Radar};
				\node[sysblock, fill=green!12, right=2.5cm of radar] (esp) {ESP32-S3\\Sensor Node};
				\node[sysblock, fill=orange!12, right=2.5cm of esp] (tag) {ESP32\\Active Tag};

				\draw[dataflow] (radar) -- node[above, font=\tiny] {UART 256k} node[below, font=\tiny, text=gray] {GPIO16/17} (esp);
				\draw[dataflow, <->] (esp) -- node[above, font=\tiny] {Wi-Fi FTM} node[below, font=\tiny, text=gray] {802.11mc} (tag);

				\node[below=0.5cm of esp, font=\tiny, text=gray] {Core 0: Wi-Fi \quad Core 1: UART + ML};

				% Reflector annotation
				\node[below=0.3cm of tag, font=\tiny, text=gray, text width=2cm, align=center] {+ Corner Reflector\\(RCS boost)};
			\end{tikzpicture}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.58\textwidth}
			\centering
			% TikZ RTOS Architecture
			\begin{tikzpicture}[scale=0.72, transform shape,
				taskbox/.style={rectangle, rounded corners=2pt, draw=#1!70, fill=#1!8, minimum width=3.8cm, minimum height=0.6cm, text width=3.6cm, align=left, font=\tiny},
				taskbox/.default=blue]

				% Core 0 box
				\node[draw=blue!60, fill=blue!3, rounded corners=4pt, minimum width=4.4cm, minimum height=3.6cm, label={[font=\small\bfseries, text=blue!80]above:Core 0}] (c0) at (0,0) {};

				% Core 1 box
				\node[draw=green!60!black, fill=green!3, rounded corners=4pt, minimum width=4.4cm, minimum height=3.6cm, label={[font=\small\bfseries, text=green!60!black]above:Core 1}] (c1) at (5.5,0) {};

				% Core 0 tasks
				\node[taskbox=blue] at (0, 0.9) {\textbf{wifi\_ftm\_task} (prio 5)\\FTM every 500\,ms, HT40};
				\node[taskbox=blue] at (0, -0.1) {\textbf{espnow\_task}\\Heartbeat monitor};
				\node[taskbox=blue, draw=red!50] at (0, -1.1) {\small\textit{Never touches UART}};

				% Core 1 tasks
				\node[taskbox=green!50!black] at (5.5, 0.9) {\textbf{uart\_radar\_task} (prio 5)\\8192-byte RX buffer};
				\node[taskbox=green!50!black] at (5.5, -0.1) {\textbf{ml\_inference\_task} (prio 3)\\TFLite Micro Float32};
				\node[taskbox=green!50!black] at (5.5, -1.1) {$\sim$350\,$\mu$s / inference};
	...
	\begin{columns}[c]
		...
		\begin{column}{0.48\textwidth}
			...
			\begin{tikzpicture}[scale=1.1, transform shape]
				...
				% FoV cone
				\fill[blue!8] (0,0) -- (60:4.2) arc (60:120:4.2) -- cycle;
				\draw[blue!40, thick] (0,0) -- (60:4.2);
				\draw[blue!40, thick] (0,0) -- (120:4.2);
				\draw[blue!30, dashed] (60:4.2) arc (60:120:4.2);

				% Boresight
				\draw[gray, thick] (0,0) -- (90:4.5);
				\node[above, font=\tiny, text=gray] at (90:4.5) {$\theta=0^{\circ}$ (boresight)};

				% Angle labels
				\node[font=\tiny, text=blue!60] at (55:3.5) {$+60^{\circ}$};
				\node[font=\tiny, text=blue!60] at (125:3.5) {$-60^{\circ}$};

				% Target point
				\fill[red!70] (65:3.2) circle (4pt);
				\node[right=3pt, font=\tiny\bfseries, text=red!70] at (65:3.2) {Target};
				\node[right=3pt, below right=-1pt and 3pt, font=\tiny, text=gray] at (65:3.2) {$(r{=}3.2\text{m},\ \theta{=}{+}25^{\circ})$};

				% Range arc
				\draw[red!40, dashed, thick] (60:3.2) arc (60:95:3.2);
				\node[font=\tiny, text=red!50] at (97:3.4) {$r$};
...
\begin{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.45\textwidth}
			\centering
			% TikZ MLP Architecture
			\begin{tikzpicture}[scale=0.85, transform shape,
				layer/.style={rectangle, rounded corners=3pt, draw=#1!70, fill=#1!12, minimum width=2.5cm, minimum height=0.7cm, font=\scriptsize\bfseries, align=center},
				layer/.default=blue]

				\node[layer=green!50!black] (input) at (0,0) {Input (25)};
				\node[layer=blue] (d1) at (0,-1.3) {Dense(48, ReLU)};
				\node[layer=blue] (d2) at (0,-2.6) {Dense(24, ReLU)};
				\node[layer=red!60!black] (out) at (0,-3.9) {Dense(2, Linear)};
				\node[font=\scriptsize, text=red!60!black] (outlbl) at (0,-4.7) {$[\hat{r}_{\text{norm}},\ \hat{\theta}_{\text{norm}}]$};
...
\begin{frame}
	...
	\begin{tikzpicture}[scale=0.85, transform shape]
		...
		\node[pipeblock=blue] (feat) at (3.3,0) {Feature\\Engineering};
		\node[pipeblock=purple] (train) at (6.6,0) {Training\\(Keras)};
		\node[pipeblock=orange] (conv) at (9.9,0) {Conversion\\(TFLite)};
		\node[pipeblock=red!60!black] (deploy) at (9.9,-2) {ESP32-S3\\TFLite Micro};
		...
		% Annotations below
		\node[annot, below=0.2cm of data, text width=2.2cm, align=center] {70 static poses\\30s each\\Micro-motion\\Polar ground truth};
		\node[annot, below=0.2cm of feat, text width=2.2cm, align=center] {Forward-fill\\Normalize [0,1]\\Window = 5\\Split by segment};
		\node[annot, below=0.2cm of train, text width=2.2cm, align=center] {EarlyStopping\\ReduceLR\\MSE loss\\200 epochs max};
		\node[annot, below=0.2cm of conv, text width=2.2cm, align=center] {Pure Float32\\No quantization\\xxd $\to$ C header\\alignas(16)};
	\end{tikzpicture}
	...
	\begin{columns}[c]
		\begin{column}{0.45\textwidth}
			\begin{block}{Dataset}
				$\sim$35,000 events across 70 segments (50 clear + 20 occluded)
			\end{block}
		\end{column}
		\begin{column}{0.45\textwidth}
			\begin{block}{Split}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.45\textwidth}
			\begin{figure}
				\includegraphics[width=\linewidth, height=0.55\textheight, keepaspectratio]{Placeholder.png}
				\caption{Training loss curve — convergence at epoch 23, EarlyStopping at epoch 38}
			\end{figure}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.55\textwidth}
			\begin{figure}
				\includegraphics[width=0.48\linewidth, height=0.45\textheight, keepaspectratio]{Placeholder.png}
				\hfill
				\includegraphics[width=0.48\linewidth, height=0.45\textheight, keepaspectratio]{Placeholder.png}
				\caption{Scatter: predicted vs ground truth ($r$ left, $\theta$ right)}
			\end{figure}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.50\textwidth}
			\begin{figure}
				\includegraphics[width=\linewidth, height=0.55\textheight, keepaspectratio]{Placeholder.png}
				\caption{ESP32-S3 serial output showing FUSED predictions with inference times}
			\end{figure}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.46\textwidth}
			...
			\begin{tikzpicture}[scale=0.8, transform shape]
				...
				% Min line
				\draw[red!70, thick, dashed] (0, 1.5) -- (5.2, 1.5) node[right, font=\tiny\bfseries, text=red!70] {Min: 2.7\,m};

				% True distance line
				\draw[green!60!black, thick, dotted] (0, 1.3) -- (5.2, 1.3) node[right, font=\tiny\bfseries, text=green!60!black] {True: 2.5\,m};

				% Mean line
				\draw[orange!70, thick, dashdotted] (0, 2.2) -- (5.2, 2.2) node[right, font=\tiny\bfseries, text=orange!70] {Mean: 3.2\,m};
			\end{tikzpicture}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\centering
	\scriptsize
	\begin{tabular}{c p{2.8cm} p{3.5cm} p{4cm}}
		\toprule
		\textbf{\#} & \textbf{Challenge} & \textbf{Root Cause} & \textbf{Solution} \\
		\midrule
		1 & Radar drops stationary targets & LD2450 MTI filter requires Doppler shift & Micro-motion protocol — subjects sway gently, feet planted \\[4pt]
		2 & UART corruption during FTM & Wi-Fi ISR preempts UART on same core, 128\,B FIFO overflows in 5\,ms & UART on Core 1, Wi-Fi on Core 0 — physically isolated \\[4pt]
		3 & INT8 quantization destroys accuracy & 256 bins / 6\,m = 2.35\,cm floor, compounds; tanh LUT 30\%+ off & Stay \alert{Float32} — 11.6\,KB, quant saves nothing \\[4pt]
		4 & Zero-fill pulls to origin & Missing $r{=}0$ means ``at sensor'' not ``unknown'' & \alert{Forward-fill} + binary freshness flag \\[4pt]
		5 & Wi-Fi multipath inflates range & 2.4\,GHz reflects, 20\,MHz can't resolve & \alert{HT40} + 16-frame min-RTT filter \\
		\bottomrule
	\end{tabular}
\end{frame}
